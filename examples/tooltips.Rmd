...
| `crosshairsX` | `FALSE` | Show vertical crosshair only (series marks only) |
| `crosshairsY` | `FALSE` | Show horizontal crosshair only (series marks only) |
| `crosshairsStroke` | `'#aaa'` | Crosshair line color |
| `crosshairsLineWidth` | `2` | Crosshair line width in pixels |
...
```{r}
p |> tooltip_(crosshairsX = TRUE)
```

```{r}
p |> tooltip_(crosshairsY = TRUE)
```
...
```{r}
p |> tooltip_(
  crosshairs = TRUE, crosshairsStroke = '#999',
  crosshairsLineWidth = 1, crosshairsLineDash = c(4, 4)
)
```
...
```{r}
p |> tooltip_(marker = FALSE)
```

```{r}
p |> tooltip_(markerType = 'hollow', markerSize = 8)
```
...
```{r}
p_bar |> mark_interval(
  tooltip = list(channel = 'y', name = 'Avg Temp', valueFormatter = '.1f')
)
```
...
```{r}
p_bar |> mark_interval(
  tooltip = list(channel = 'y', name = 'Avg Temp', color = 'steelblue')
)
```
