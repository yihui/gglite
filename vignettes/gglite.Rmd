...
whichever you prefer: `|>` is idiomatic R (>= 4.1), while `+` mirrors the
ggplot2 style.
...
| categorical | numeric | `boxplot` | Box plot |
| numeric | categorical | `boxplot` (transposed) | Horizontal box plot |
...
### Horizontal box plot (numeric × categorical)
...
```{r}
g2(sunspot.year) |> title_('Yearly Sunspot Numbers (1700--1988)')
```

Multivariate time series produce one line per series:

```{r}
g2(EuStockMarkets) |> title_('EU Stock Markets (1991--1998)')
```
