...
```{r}
g2(sunspot.year) |> header('Yearly Sunspot Numbers (1700--1988)')
```

Multivariate time series produce one line per series:

```{r}
g2(EuStockMarkets) |> header('EU Stock Markets (1991--1998)')
```
...
```{r}
g2(mtcars, hp ~ mpg) |>
  header('Motor Trend Cars', subtitle = 'mpg vs horsepower')
```
...
g2(df, y ~ x, color = ~ product) |>
  ...
  header('Quarterly Sales', subtitle = 'By product line') |>
  interact('tooltip') |>
