...
```{r}
g2(iris, Sepal.Length ~ Sepal.Width | Species) |> mark_point()
```
...
```{r}
g2(mtcars, ~ mpg) |>
  mark_interval(transform = list(list(type = 'binX', y = 'count')))
```
...
| categorical | numeric | `interval` | Bar chart |
| numeric | categorical | `interval` (transposed) | Horizontal bar |
| categorical | categorical | `cell` | Heatmap |
| Date | numeric | `line` | Line chart |
| numeric | _(none)_ | `interval` + `binX` | Histogram |
| categorical | _(none)_ | `interval` + `groupX` | Count bar chart |
| _(position)_ | | `line` + parallel | Parallel coordinates |
